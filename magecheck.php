...
$test->addSection('Nginx');

if( strpos( $_SERVER["SERVER_SOFTWARE"], 'ngix' ) !== false ) {
	   
	$message = new Magecheck_Test_ResultMessage("PHP is  running as a Nginx module.");
    $test->addResult('Nginx', $message);
   
}else {
	$message = new Magecheck_Test_ResultMessage("PHP is not running as a Nginx module so it cannot detect which modules are enabled.");
    $test->addResult('Nginx', $message);
}
